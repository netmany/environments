...
(define (set-lisp-indent-number sym num-of-subforms-to-be-indented-wide)
    (let* ((x (symbol->string sym))
           (c (assp (lambda (a) (string-ci=? x a)) *lisp-keywords*)))
        (unless c
                (set! c (cons x (box 0)))
...
(define (set-indent num syms)
    (for-each (lambda (s) (set-lisp-indent-number s num))
              syms))
...
(define (lisp-indent-number s)
    (let ((c (assp (lambda (a) (string-ci=? s a)) *lisp-keywords*)))
        (cond (c (unbox (cdr c)))
                   ...
                   (let ((w (substring s i j)))
                       (if (and (>= i 2)
                           ...
                           (let ((nas (lisp-indent-number w)))
                               (cond ((>= nas 0) (set! num-aligned-subforms nas) 2)
                                     ((literal-token? w) 1)
                                     ((= j n) 1)
                                     (else (+ (- j i) 2)))))))))
...
(define (indent-lines)
    (let ((left-i 0) (paren-stack '()) (stringp #f))
        (let line-loop ()
            (let ((curr-line (get-line (current-input-port))))
